...
Matrix Matrix::inv(Matrix M) {
	...
	for (int i = 0; i < M._matrix_row_; i++) {
		for (int j = 0; j < M._matrix_column_; j++) {
			printf("%7.4lf\t\t", M._matrix_head_pointer_[i][j]);
		}
		std::cout << std::endl;
	}
	M = Temp;//还原A
...
Matrix Matrix::linspace(double first_val, double last_val, int linspace_dim, linspace_type type)
{
	double d = (last_val - first_val) / (double)linspace_dim;
	if (linspace_dim <= 0)
	{
	...
	{
		Matrix Temp1(linspace_dim, 1);
		Temp1._matrix_head_pointer_[linspace_dim - 1][0] = last_val;		//赋值最后一项
		for (int i = 0; i < linspace_dim - 1; i++)
		{
	...
	{
		Matrix Temp2(1, linspace_dim);
		Temp2._matrix_head_pointer_[0][linspace_dim - 1] = last_val;		//赋值最后一项
		for (int i = 0; i < linspace_dim - 1; i++)
		{
