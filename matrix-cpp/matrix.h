...
friend std::ostream&	operator<<(std::ostream&, const Matrix&);
	...
	//构造长度为_order的零方阵
	static	Matrix		zeros(int _order);
	//构造_row*col的零矩阵
	static	Matrix		zeros(int _row, int _col);
	//构造维度为_order的1方阵
	static	Matrix		ones(int _order);
	//构造_row*col的1矩阵	
	...
	static	Matrix		linspace(double first_val, double last_val, int linspace_nums,linspace_type type);
...
public:
	...
	SquareMatrix(int _order);
	SquareMatrix(int _order, double val);
